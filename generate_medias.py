...
# Plot the grid
fig, ax = plt.subplots(figsize=(14, 10))  # Increased width for wider cells
...
cbar.set_ticklabels([f"{x}%" for x in xticks])
...
ax.set_yticklabels(['Window-side', 'Middle', 'Aisle-side', '', 'Aisle-side', 'Middle', 'Window-side'])
ax.set_xticks(np.arange(rows))
ax.set_xticklabels(np.arange(1, rows + 1))

# Add probability values in each cell
for r in range(rows):
    for c in range(cols):
        if grid[c, r] > 0:
            ax.text(r, c, f'{grid[c, r]:.1f}%', ha='center', va='center', color='black')
